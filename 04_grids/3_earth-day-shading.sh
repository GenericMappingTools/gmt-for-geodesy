...
#   gmt basemap -JM15c -B+n -RUN005     # South America (Based on list of countries by the UN)
...
#   1. Calculate intesity grids from the relief grid 
    ...
    gmt grdgradient @earth_synbath_${RES} -Grelief_gradient.nc -A45 -Nt1

#   2. Plot satelital image with hill shading efect (from the grid created in 1)
    gmt grdimage @earth_day_${RES} -Irelief_gradient.nc
