...
gmt begin 5_exercise png
    ...
    # 1. Set a Mercator projection, and region for Argentina.
    gmt basemap -RAR,FK,GS+r2 -JM15c -B+n
    
    # 2. Set the default freme (-Baf) and add a title to the plot (-B+t).
    ...
    # 6. Plate a colorbar inside ner the right top with and offset in x (+o) and
